...
- Is the single source of truth for the aspect it defines
...
Declarative configuration is the documentation. No separate documentation is written to describe what the configuration already expresses.
...
Each system is brought into existence from its declarative configuration. The creation method is determined by the nature of the system and may be:

- **Automatic:** the configuration is applied directly (e.g. Terraform)
- **Semi-automatic:** some steps are automated, others are not
- **Manual:** the configuration describes the desired state and a human performs the required actions
